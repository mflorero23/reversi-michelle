...
io.on('connection', (socket) => {
   ...
   /* join_room_response:
    ...
    socket.on('join_room', (payload) => {
        ...
        if ((typeof payload == 'undefined') || (room === null)){
            response = {};
            response.result = 'fail';
...
/*june 9 7:12 pm left off here for QA */
    ...
    socket.on('game_start', (payload) => {
        ...
        if ((typeof requested_user == 'undefined') || (requested_user === null) || (requested_user === "")) {
            ...
            socket.emit('game_start_response', response);
            serverLog('game_start failed', JSON.stringify(response));
            return;
        } 
        if ((typeof room == 'undefined') || (room === null) || (room === "")) {
            response = {
        ...
        /* Make sure that the invited player to engage is present */
        io.in(room).allSockets().then((sockets)=> {
...
function check_line_match(color, dr, dc, r, c, board){
    ...
    return(check_line_match(color, dr, dc, r+dr, c+dc, board)); /*color comes before dr?*/
}
